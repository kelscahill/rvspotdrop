<?php
class WPForms_Field_Select_state extends WPForms_Field {
...
//	var_dump("dasad");
		...
		$this->name     = esc_html__( 'State Dropdown', 'wpforms-lite' );
		$this->type     = 'select_state';
		$this->icon     = 'fa-caret-square-o-down';
		$this->order    = 1;
		$this->defaults = array(
			1 => array(
				'label'   => esc_html__( 'Select State', 'wpforms-lite' ),
				'value'   => '',
				'default' => '',
			),
    ...
    public function field_option( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			...
			case 'css':
				...
				$output .= $this->field_element( 'text', $field, array( 'slug' => 'css', 'value' => 'csca_wp_states'), false );
...
public function field_preview_option( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			...
			case 'choices':
				$fields_w_choices = array( 'checkbox', 'gdpr-checkbox', 'select','select_state', 'payment-select', 'radio', 'payment-multiple', 'payment-checkbox' );
...
new WPForms_Field_Select_state();
?>